                        ...
                        <table class="table table-bordered" id="supplierEditTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>Amount</th>
                                    <th>Item Image</th>
                                    <th><button type="button" class="btn btn-sm btn-primary" id="add-row">Add Item</button></th>
                                </tr>
                            </thead>
                            <tbody>
                                @if ($supplier->items && count($supplier->items))
                                    @foreach ($supplier->items as $key => $item)
                                        <tr>
                                            ...
                                            <td><input type="number" name="item_price[]" class="form-control" step="0.01" value="{{ $item->item_price }}"></td>
                                            <td><input type="number" name="item_amount[]" class="form-control" step="0.01" value="{{ $item->item_amount }}"></td>
                                            <td>
                                                @if($item->item_image)
                                                    <img src="{{ asset('storage/items/' . $item->item_image) }}" width="60" class="mb-1">
                                                @endif
                                ...
                                @else
                                    <tr>
                                        <td colspan="6" class="text-center text-muted">No items listed for this supplier.</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
